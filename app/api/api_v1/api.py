...
from utils.helpers import calc_tax_from_total
from pyfatoora.schemas import InvoiceData
...
async def get_base64_endpoint(
    ...
    tax_amount = calc_tax_from_total(total, tax_amount)

    fatoora = PyFatoora(seller_name, tax_number, date, total, tax_amount)
...
async def post_qrcode_image_endpoint(
    invoice_data: InvoiceData, background_tasks: BackgroundTasks
):
...
def handle_form(
    ...
    if render_type == "3":
        ...
        return templates.TemplateResponse(
            "fatoora.html", {"request": request, "data": data}
        )
